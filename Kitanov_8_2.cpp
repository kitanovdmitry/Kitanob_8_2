...
void Add_Elem(our_struct*& address, int val1, int val2, int val3)
{
    ...
    for (size_t i = 0; i < NumElems; ++i)
    {
        ...
        // ѕровер€ть на ptrs[i] на NULL не имеет смысла, поскольку при недостатке пам€ти
    ...
    // вставл€ем новую €чейку в цепочку €чеек
    if (address == nullptr)
    {
        address = ptrs[0];
    }
    else
    {
        ...
        // спускаемс€ по левым элементам до начала следующей €чейки
        while (p->left->left != nullptr)
        {
            p = p->left->left;
        }
        // настраиваем указатели последней €чейки на первый элемент новой €чейки
